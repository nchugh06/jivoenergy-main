...
export default function EditJobPage() {
  ...
  const [item, setItem] = useState<JobOpening | null>(null);
    ...
    const load = async () => {
      try {
        ...
        if (data.item?.deletedAt) {
          alert('Restore this job before editing');
          router.push('/admin/jobs');
          return;
        }
        setItem(data.item);
  ...
  const handleUpdate = async (data: JobWritePayload, pdfFile: File | null) => {
    setSubmitting(true);
    try {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-8">
        ...
        <h1 className="section-title-spl text-gray-800">Edit Job Opening</h1>
        <p className="text-gray-500 mt-1">Update listing details or publish status</p>
      </div>
