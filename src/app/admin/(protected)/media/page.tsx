...
export default function AdminMediaPage() {
  ...
  const handleSeed = async () => {
    if (!confirm('Import items from news.json? Existing slugs will be updated; items not in JSON will be kept.')) {
      return;
    }
    setSeeding(true);
    try {
      const res = await adminFetch('/api/admin/media/seed', { method: 'POST', body: '{}' });
      const data = await res.json();
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex gap-4">
          ...
          >
            ...
            {seeding ? 'Importing...' : 'Import from JSON'}
          </button>
          <Link
